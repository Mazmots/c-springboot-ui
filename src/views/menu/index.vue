...
export default {
  ...
  methods: {
    ...
    onSubmit(){
      let param = JSON.parse(JSON.stringify(this.state.ruleForm))
      Object.assign(param, JSON.parse(JSON.stringify(this.state.ruleForm.meta)))

      const rolesString = param.roles.join(',')
      param.roles = rolesString
      
      // 新增或修改
      if (this.state.dialog.submitTxt = '修 改') {
        useMenuApi().update(param).then(response => {
        ...
        useMenuApi().add(param).then(response => {
